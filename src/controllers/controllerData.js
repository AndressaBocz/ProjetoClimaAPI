...
exports.enviaCidade = (req, res) => {
  ...
  axios
    .get(
      `https://api.openweathermap.org/data/2.5/weather?q=${cidade}&appid=${'443dccd16407409eea0bf04a04160fb9'}`
    )
    .then((response) => {
...
exports.enviaHora = (req, res) => {
  ...
  axios
    .get(
      `http://api.timezonedb.com/v2.1/get-time-zone?key=${'43WQWGKYRDRL'}&format=json&by=position&lat=${lat}&lng=${lon}`
    )
    .then((response) => {
      res.send(response.data);
    })
